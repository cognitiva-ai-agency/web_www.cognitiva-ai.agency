...
const UNIFIED = {
  ...
  "Finanzas": {
    ...
    metrics: { approvals: "3×", processing: "-75%", fraud_detection: "99.5%" }
  },
  "Turismo": {
      ...
      <div ref={ref} className="container-padded relative animate-in">
              ...
              >
                ...
                <div 
                  className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${gradient} transition-all duration-500 ${
                    isExpanded ? 'opacity-20 blur-xl' : 'opacity-0'
                  } ${isHovered && !isExpanded ? 'opacity-10 blur-lg' : ''}`} 
                />
                
                {/* Card principal con altura dinámica */}
                <div className={`relative h-full rounded-3xl bg-gradient-to-br from-white/[0.06] to-white/[0.02] backdrop-blur-xl border transition-all duration-500 ${
                  isExpanded ? 'border-cyan-400/30' : 'border-white/10'
                } ${isHovered ? 'translate-y-[-4px] shadow-[0_20px_40px_rgba(59,130,246,0.15)]' : ''}`}>
                  
                  {/* Header clickeable */}
                  <button
                    onClick={() => toggleCard(title)}
                    className="w-full p-6 md:p-8 text-left focus:outline-none focus:ring-2 focus:ring-cyan-400/50 rounded-t-3xl"
                    aria-expanded={isExpanded}
                    aria-controls={`industry-content-${index}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        {/* Icono con efecto */}
                        <div className="relative">
                          <div className={`absolute inset-0 bg-gradient-to-r ${gradient} rounded-2xl blur-xl transition-all duration-500 ${
                            isExpanded ? 'opacity-60' : 'opacity-25'
                          } ${isHovered ? 'opacity-45' : ''}`} />
                          <div className={`relative w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border border-white/20 flex items-center justify-center transition-all duration-500 ${
                            isExpanded ? 'scale-110 rotate-3' : ''
                          } ${isHovered ? 'scale-105' : ''}`}>
                            <Icon className={`h-7 w-7 md:h-8 md:w-8 text-white transition-all duration-500 ${
                        ...
                        {/* Título y KPI */}
                        <div>
                          <h3 className="text-xl md:text-2xl font-light text-white tracking-tight">{title}</h3>
                          {!isExpanded && (
                  ...
                  >
                    <div className="px-6 md:px-8 pb-6 md:pb-8">
                      ...
                      {/* KPI destacado cuando está expandido */}
                      {isExpanded && (
                        <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-white/5 to-white/[0.02] border border-white/10">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-light text-blue-200/70">Resultado principal</span>
                            <div className="flex items-center gap-2">
                              <Rocket className="h-4 w-4 text-cyan-400" />
                              <span className="text-lg font-light text-white">{kpi}</span>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {features && (
                        <div className="mb-6">
                          <h4 className="text-xs font-medium uppercase tracking-wider text-cyan-400 mb-3">
                          ...
                          <div className="space-y-2">
                            {features.map((feature, idx) => (
                              <div key={idx} className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-light text-blue-100/80">{feature}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {metrics && (
                        <div className="mb-6 grid grid-cols-3 gap-2">
                          {Object.entries(metrics).map(([key, value]) => (
                            <div key={key} className="text-center p-2 rounded-lg bg-gradient-to-r from-white/5 to-white/[0.02] border border-white/10">
                              <div className="text-sm font-light text-cyan-300">{value}</div>
                              <div className="text-xs text-blue-200/50">{key.replace('_', ' ')}</div>
                            </div>
                          ))}
                        </div>
                      )}
        ...
        <div className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "Casos de éxito", value: "500+", icon: CheckCircle2 },
